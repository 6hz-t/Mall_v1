              ...
              <el-row>
                <el-col :span="12">
                  ...
                  <span v-if="materialgroupObj.id != '-1'">
                    <el-button size="small" type="text" class="el-icon-delete" style="margin-left: 10px;color: red" @click="groupDelete(materialgroupObj)">删除</el-button>
                  </span>
                </el-col>
...
<script>
import { getPage as groupPage, add as groupAdd, del as groupDel, update as groupEdit } from '@/api/common/photoGroup'
import { getPage, add, del, update } from '@/api/common/photo'
  ...
  methods: {
    ...
    groupPage() {
      ...
      groupPage({
        ...
        const materialgroupList = response.data.data
    ...
    handleSuccess(response, file, fileList) {
      ...
      add({
        type: '1',
        photoGroupId: this.groupId !== '-1' ? this.groupId : -1,
        name: file.name,
    ...
    sureUrls() {
      ...
      this.$emit('select', this.index, this.urls)
